...
func main() {
	...
	flag.StringVar(&kafkaTopic, "kafka-topic", "topic1", "Kafka topic to push")

	flag.Parse()

	// connect to kafka
	kafkaProducer, err := kafka_producer.Configure(strings.Split(kafkaBrokerUrl, ","), kafkaClientId, kafkaTopic)
	if err != nil {
